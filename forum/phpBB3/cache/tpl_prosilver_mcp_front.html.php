<?php $this->_tpl_include('mcp_header.html'); ?>
			...
			<li class="row<?php if (($_unapproved_val['S_ROW_COUNT'] & 1)  ) {  ?> bg1<?php } else { ?> bg2<?php } ?>">
				<dl>
					<dt>
						...
						<?php echo ((isset($this->_rootref['L_POSTED'])) ? $this->_rootref['L_POSTED'] : ((isset($user->lang['POSTED'])) ? $user->lang['POSTED'] : '{ POSTED }')); ?> <?php echo ((isset($this->_rootref['L_POST_BY_AUTHOR'])) ? $this->_rootref['L_POST_BY_AUTHOR'] : ((isset($user->lang['POST_BY_AUTHOR'])) ? $user->lang['POST_BY_AUTHOR'] : '{ POST_BY_AUTHOR }')); ?> <?php if ($_unapproved_val['U_AUTHOR']) {  ?><a href="<?php echo $_unapproved_val['U_AUTHOR']; ?>"><?php echo $_unapproved_val['AUTHOR']; ?></a><?php } else { echo $_unapproved_val['AUTHOR']; } ?> <?php echo ((isset($this->_rootref['L_POSTED_ON_DATE'])) ? $this->_rootref['L_POSTED_ON_DATE'] : ((isset($user->lang['POSTED_ON_DATE'])) ? $user->lang['POSTED_ON_DATE'] : '{ POSTED_ON_DATE }')); ?> <?php echo $_unapproved_val['POST_TIME']; ?>
					</dt>
...
<?php } if ($this->_rootref['S_SHOW_LOGS']) {  ?>
	...
	<?php }} else { ?>
		<tr>
			<td colspan="5"><?php echo ((isset($this->_rootref['L_NO_ENTRIES'])) ? $this->_rootref['L_NO_ENTRIES'] : ((isset($user->lang['NO_ENTRIES'])) ? $user->lang['NO_ENTRIES'] : '{ NO_ENTRIES }')); ?></td>
		</tr>
	<?php } ?>
		</tbody>
		</table>
