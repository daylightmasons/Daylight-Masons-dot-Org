...
<?php } else if ($this->_rootref['S_EDIT_IMAGESET']) {  ?>
	...
	// <![CDATA[
		...
		{
			document.getElementById('newimg').src = (newimage) ? '../styles/<?php echo (isset($this->_rootref['A_NAME'])) ? $this->_rootref['A_NAME'] : ''; ?>/imageset/' + encodeURI(newimage) : 'images/no_image.png';
		}
	// ]]>
	</script>
...
<?php } else if ($this->_rootref['S_EDIT_TEMPLATE'] || $this->_rootref['S_EDIT_THEME']) {  ?>
	...
	<?php $_file_count = (isset($this->_tpldata['file'])) ? sizeof($this->_tpldata['file']) : 0;if ($_file_count) {for ($_file_i = 0; $_file_i < $_file_count; ++$_file_i){$_file_val = &$this->_tpldata['file'][$_file_i]; if (!($_file_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="row1"><?php } else { ?><tr class="row2"><?php } ?>
			<td><a href="<?php echo $_file_val['U_VIEWSOURCE']; ?>" onclick="popup(this.href, 750, 550, '_source'); return false;"><?php echo $_file_val['FILENAME']; ?></a></td>
...
<?php } else if ($this->_rootref['S_FRONTEND']) {  ?>
	...
	<dl>
		...
		<dd><?php if ($this->_rootref['S_INSTALL']) {  ?><b id="copyright"><?php echo (isset($this->_rootref['COPYRIGHT'])) ? $this->_rootref['COPYRIGHT'] : ''; ?></b><?php } else { ?><input type="text" id="copyright" name="copyright" value="<?php echo (isset($this->_rootref['COPYRIGHT'])) ? $this->_rootref['COPYRIGHT'] : ''; ?>" /><?php } ?></dd>
	</dl>
